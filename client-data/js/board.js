...
Tools.toolbar = document.getElementById("toolbar");
...
if (isEdit !== null) {
  Tools.svg.style.margin = 0;
  bodyElement.style.overflow = "hidden";
	bodyElement.style.backgroundColor = "#FFF";
	htmlElement.style.backgroundColor = "#FFF";
	
  if (isEdit === "false") {
    Tools.toolbar.style.display = "none";
  }
} else {
  Tools.svg.style.margin = `${
    (boardWrapper.clientHeight -
      Math.round(Tools.server_config.MAX_BOARD_SIZE_Y * 0.2)) /
    2
  }px ${
    (boardWrapper.clientWidth -
      Math.round(Tools.server_config.MAX_BOARD_SIZE * 0.2)) /
    2
  }px`;
...
Tools.add = function (newTool) {
  ...
  if (
    newTool.moderator &&
    document.getElementById("moderator").text !== "true"
  ) {
    return;
  }
...
Tools.change = function (toolName) {
  ...
  if (newTool === oldTool) {
    if (newTool.secondary) {
      ...
      Tools.svg.style.cursor =
        newTool.secondary.active && newTool.secondary.mouseCursor
          ? newTool.secondary.mouseCursor
          : newTool.mouseCursor;
    }
    return;
  }
...
function resizeCanvas(m) {
  ...
  if (y > Tools.svg.height.baseVal.value - 2000) {
    Tools.svg.height.baseVal.value = Math.min(
      y + 2000,
      Tools.server_config.MAX_BOARD_SIZE_Y
    );
  }
}
...
Tools.setScale = function setScale(scale) {
  ...
  Tools.svg.style.willChange = "auto"; // grid transforms smooth and don't hide
  Tools.svg.style.transform = "scale(" + scale + ")";
  Tools.board.style.width =
    Math.round(Tools.server_config.MAX_BOARD_SIZE * scale).toString() + "px";
  Tools.board.style.height =
    Math.round(Tools.server_config.MAX_BOARD_SIZE_Y * scale).toString() + "px";
...
(function () {
  ...
  let tokenObject = JSON.parse(
    decodeURIComponent(escape(window.atob(Tools.token.split(".")[1])))
  );
