...
(function () { //Code isolation
	...
	function continueLine(x, y, evt) {
		...
		if (curLine !== null) {
			...
			if (performance.now() - lastTime > 70) {
				Tools.draw(new UpdateMessage(x, y));
	...
	function stopLine(x, y) {
		// Удаляем текущую линию при отпускании кнопки мыши
		if (curLine !== null) {
			var line = svg.getElementById(curLine.id);
			// if (line) {
			//     line.remove();
			// 		Tools.lineLength = 0;
			// }
	...
	function draw(data) {
		switch (data.type) {
			...
			case "update":
				...
				Tools.lineLength = Math.round(
					Math.sqrt(
						Math.pow(+line.attributes.x2.value - +line.attributes.x1.value, 2) +
						Math.pow(+line.attributes.y2.value - +line.attributes.y1.value, 2)
					)
				);
				...
				text.textContent = Tools.lineLength;
